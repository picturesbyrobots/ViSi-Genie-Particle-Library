...
// This Demo communicates with 2 4D Systems Displays, configured with ViSi-Genie, utilising the Genie Particle Library - https://github.com/4dsystems/ViSi-Genie-Particle-Library
...
// The User LED is updated by the Particle, by first doing a manual read of the User LED and then toggling it based on the state received back.

// As the slider changes, it sends its value to the Particle (Particle also polls its value using genie.ReadObject, as above), and the Particle then
// tells the LED Digit to update its value using genie.WriteObject, but of the other displays LED Digit! So the Slider message goes via the Particle to the LED Digit
// of the other display.
// Coolgauge is updated via simple timer in the Particle code, and updates the display with its value.
...
void setup()
{
  ...
  // 200K Baud is good for most high speed comms.
  ...
  pinMode(RESETLINE1, OUTPUT);  // Set D4 on Particle to Output to control the reset line to Display 1
  pinMode(RESETLINE2, OUTPUT);  // Set D2 on Particle to Output to control the reset line to Display 2
